function [x,h,D1,D2] = FDDiffMat( domain,N,stencilSize,direction,option)

% direction : CD (central), FD (forward), BD (backward)

periodic = ~strcmp('non-periodic',option);

h = (domain(2)-domain(1))/N;
x = domain(1)+(0:N-periodic)*h;

if strcmp(direction,'CD') % centered finite difference
    orderAccuracy = stencilSize - 1;
    domainStencil = -orderAccuracy/2:orderAccuracy/2;
elseif strcmp(direction,'FD') % forward finite difference
    orderAccuracy = stencilSize -1;
    domainStencil = 0:stencilSize-1;
elseif strcmp(direction,'BD') % backward finite difference
    orderAccuracy = stencilSize -1;
    domainStencil = -(stencilSize-1:-1:0);
end

[D1, D2] = deal(nan(N+~periodic));

if length(domainStencil) > 1
    D1 = diffMatix(1, orderAccuracy,N,periodic,domainStencil);
end

if length(domainStencil) > 2
    D2 = diffMatix(2, orderAccuracy,N,periodic,domainStencil);
end

end

function T = diffMatix(derivativeOrder, orderAccuracy,N,periodic,domainStencil)
N = N+~periodic;
Nzero = N - orderAccuracy - 1;
stcen = fdcoeffF(derivativeOrder,0,domainStencil);
rtop = [stcen zeros(1,Nzero)];
ctop = circshift(fliplr(rtop),[1 1]);
T = toeplitz(ctop,rtop);
T(end,:) = T(end-~periodic,:);
